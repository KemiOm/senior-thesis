...
The approach is defined by the pipeline in this repository: extract and normalize source texts, annotate them with formal structure like meter, stress, rhyme, lineation, export to SQLite, then build train/dev/test splits and **four parallel task conditions** like natural_text, meter_only, rhyme_only, combined, so the same backbone model can be evaluated and trained under different target formats. Nothing in the *engineering* assumes a single literary genre beyond what the corpus contains—the same machinery applies to any line-level annotations stored in `corpus.db`. The workflow is: **prompt-based evaluation** on shared test JSONs (zero/one/few-shot; **scripts/run_prompt_eval.py** for both Hub models and merged SFT checkpoints) to **metrics / comparison** to **supervised fine-tuning (SFT)** per task with matched hyperparameters to  **re-eval** checkpoints on the same test JSONs. Gold labels always come from the annotation pipeline. The rest of this document walks through each stage and the code paths that implement it.
...
- **rhyme_only:** input = line text; target = rhyme key from phonology which is the last stressed syllable onward or rhyme_group.
...
**What it’s for.**  
When I use models with prompting only, there are no task-specific weight updates—they just follow instructions and examples without learning my labels. SFT lets me test: if I **train** the model on `train.json` / `dev.json` for each task (`meter_only`, `rhyme_only`, `combined`), does it **predict the gold labels better** than the same backbone with prompting alone? I run a **separate LoRA training for each task** so I can compare conditions fairly (meter-trained vs rhyme-trained vs combined-trained), instead of mixing everything together.

**What happens.**  
I run `scripts/sft/lora_train.py` on a GPU (via `scripts/hpc/lora_train.slurm` on Bouchet). It reads the same training data (`output/training_data/<task>/{train,dev}.json`), fits LoRA adapters on top of the frozen base model, and writes outputs under `--output-root` (default: `sft/<task>_lora/`). I only need **one** main training directory; something like `sft_full/` is optional if I want to separate longer runs. With `MERGE_AND_SAVE=1`, it saves a `final_model_merged/` folder so I can load the model directly with `from_pretrained` during evaluation.

**How I evaluate it.**  
I use the same script as prompt baselines: `run_prompt_eval.py`. I point `--model` to the merged folder, keep the same `--task` and `--prompt`, and save outputs to `results/<short_slug>/` (or set `--results-dir`). This directory only stores small JSON evaluation outputs—not checkpoints—so I keep it separate from `sft/`. I make sure `--max_tokens` is large enough to match the training `max_target_len` so outputs are not truncated.

I can also use `scripts/hpc/sft_eval.slurm` to batch evaluation across the base model and all fine-tuned variants.

Then I aggregate results with:

**Getting outputs home.**  
I use `rsync` to pull both my training outputs (usually `sft/`) and evaluation results (`results/`) from `~/Senior-Thesis/` on Bouchet to my local machine (VPN if needed).

**Other Slurm scripts.**
- `submit_lora3.sh`: submits three training jobs (one per task), defaulting to `sft/<task>_lora`
- I only override `OUTPUT_ROOT` if I want a different directory (e.g., `sft_full/<task>_lora`)
- I pass merged model paths into `sft_eval.slurm` using `METER_FT`, `RHYME_FT`, and `COMBINED_FT`
- `baseline_cpu.slurm` / `submit_baselines.sh`: run CPU prompt baselines only, not training
