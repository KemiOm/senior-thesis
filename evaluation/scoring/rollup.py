#!/usr/bin/env python3
"""
Roll prompt-eval JSON trees into model_comparison.csv.

Reads eval JSON under ``--baseline-dir``: one level ``<slug>/*.json`` or nested
``<round>/<task>/*.json`` (e.g. ``round1/combined``).

**evaluation/baseline_report/** is intended for the **pretrained (prompt-only) Hub**
runs aggregated from **evaluation/baselines/**. If you pass ``--baseline-dir results``
(or any SFT output tree), use a different ``--out-dir`` (e.g. ``results/`` or
``evaluation/baseline_report_reeval_*``) so you do not overwrite that pretrained table.

Written narrative for pretrained baselines lives in **evaluation/baseline_report/MODEL_SELECTION.MD**
(version-controlled; not generated by this script).
"""
