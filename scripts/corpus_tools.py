...
MAX_METER_BARS = 12
DEFAULT_NT_CSV = ROOT / "evaluation" / "results" / "model_comparison.csv"
DEFAULT_RANK_OUT = ROOT / "evaluation" / "results" / "natural_text_form_ranking.txt"
...
def cmd_nt_ranking(args: argparse.Namespace) -> int:
...
def main() -> None:
    ...
    p_r = sub.add_parser("nt-ranking", help="Rank natural_text models by form metrics")
    p_r.add_argument("--csv", type=Path, default=DEFAULT_NT_CSV)
    p_r.add_argument("--out", type=Path, default=DEFAULT_RANK_OUT)
    p_r.set_defaults(_run=cmd_nt_ranking)
