...
#
# Uses scripts/hpc/run_lora_train.slurm → scripts/sft/lora_train.py for each task.
...
#   bash scripts/hpc/submit_lora_three_conditions.sh
#
# Optional env (applied to every submission), e.g. fp32 if you see NaN grads:
#   NO_FP16=1 NO_BF16=1 MERGE_AND_SAVE=1 bash scripts/hpc/submit_lora_three_conditions.sh
#
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$ROOT"

SLURM="${SLURM:-scripts/hpc/run_lora_train.slurm}"
if [[ ! -f "$SLURM" ]]; then
