#!/usr/bin/env python3
"""
Aggregate prompt-only baseline JSONs: exact-match rate (gold_target vs model_output).

Writes:
  - evaluation/results/model_comparison.csv  (one row per JSON file; full audit)
  - evaluation/results/model_comparison_fair.csv  (unless --fair-n all: rows comparable per prompt_type×task)
  - evaluation/results/model_selection_notes.txt  (rankings from fair rows by default)

Fair comparison: baseline jobs must use the same `--n` (line cap) per task for each model, or
percentages will not be comparable. The default `--fair-n min` keeps only rows whose `n_scored`
equals the minimum `n_scored` seen for that (prompt_type, task), dropping longer runs until you
re-baseline with a common N (e.g. `N=500 ./scripts/submit_all_model_baselines.sh` for all models).

Usage (from project root):
  python scripts/summarize_prompt_baselines.py
  python scripts/summarize_prompt_baselines.py --print-pivot
  python scripts/summarize_prompt_baselines.py --prompt-type few_shot --print-pivot
  python scripts/summarize_prompt_baselines.py --prompt-type all --print-pivot
  python scripts/summarize_prompt_baselines.py --fair-n all          # rankings use all rows (legacy)
  python scripts/summarize_prompt_baselines.py --fair-n 500 --print-pivot  # only n_scored==500

  # Include CMU form metrics for natural_text rows (slow; use --form-max-n to cap)
  python scripts/summarize_prompt_baselines.py --include-natural-text-form --form-max-n 2000

  # Full natural_text rows + relaxed OOV (archaic poetry); long run
  python scripts/summarize_prompt_baselines.py --include-natural-text-form --relax-form-oov --prompt-type all

  # Inspect broken generations before trusting form metrics
  python scripts/helpers/diagnostic/spotcheck_natural_text_outputs.py path/to/natural_text.json --phon-check

  # Skip cheap structured partials (rhyme_only / combined field-level); on by default
  python scripts/summarize_prompt_baselines.py --skip-structured-partial
"""
...
BASELINE_DIR = ROOT / "evaluation" / "results" / "baselines" / "prompt_only"
...
def collect_rows(
    ...
    if not BASELINE_DIR.is_dir():
        print(f"Missing directory: {BASELINE_DIR}")
        return rows
...
def write_selection_notes(rows: list[dict], prompt_type: str) -> None:
    lines: list[str] = []
    lines.append("# Model selection notes (auto-generated)")
    lines.append("")
    ...
    if has_struct:
        lines.append(
            ...
            "`combined` parses `meter:…|rhyme:…|end:…|caesura:…` and reports per-field and mean-field rates "
            "(`evaluation/structured_baseline_metrics.py`)."
        )
    lines.append("")
    lines.append(
        f"Primary prompt focus for this file: **`{prompt_type}`** "
        "(see `--prompt-type`; use `all` for one section per prompt). `model_comparison.csv` still lists every JSON."
    )
    lines.append("")
    ...
    if prompt_type == "all":
        ...
        if not sub:
            lines.append(f"(No JSONs with prompt_type=`{prompt_type}` under baselines/prompt_only/.)")
            lines.append("")
            lines.append(
                "Tip: on the cluster run `./scripts/submit_all_model_baselines.sh` (default includes "
    ...
    lines.append("1. Pick **one seq2seq** checkpoint for SFT if your pipeline uses encoder–decoder (e.g. FLAN-T5, BART).")
    ...
    if complete_means_for_heuristic:
        ...
        if max_all <= 0.0:
            lines.append(
                ...
                "if that matches `train_sft_seq2seq_sample.py`; otherwise pick a causal instruct model with stable runs."
            )
    else:
        pivot_any = {}
        for pt in PROMPT_ORDER:
